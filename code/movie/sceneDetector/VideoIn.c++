...
// VideoIn.c++
//	
//	An object to handle opening a video path and delivering frames.
//
...
#define BUFFER_FRAME_COUNT 10

////////////////////////////////////////////////////////////////////////
//
// videoIn implementation class
//
////////////////////////////////////////////////////////////////////////

class videoIn : public VideoIn
{
public:
    //
    ...
    VLBoolean getValid() { return myValid; }
...
private:
    ...
    VLNode	myDrain;
    VLBuffer	myBuffer;
    VLInfoPtr	myCurrentInfo;	// info about the current frame
    stamp_t	myCurrentMSC;
    VLBoolean	myFirstFrame;
    stamp_t	myFirstMSC;
};
...
{
    //
    // Set up non-values so we know what to clean up if something fails.
    //
    ...
    myServer = vlOpenVideo( "" );
    if ( myServer == NULL ) {
	return;
    }
    
    //
    // Make the source and drain nodes: video to memory.
    //

    mySource = vlGetNode( myServer, VL_SRC, VL_VIDEO, VL_ANY );
    myDrain  = vlGetNode( myServer, VL_DRN, VL_MEM,   VL_ANY );
    
    //
    // Create the path
    //

    myPath = vlCreatePath( myServer, VL_ANY, mySource, myDrain );
    if ( myPath == -1 ) {
	return;
    }
    
    //
    // Set the sharing policy on the path.  We don't want to bother
    // handling preemption.
    //

    if ( vlSetupPaths( myServer, &myPath, 1, VL_LOCK, VL_LOCK ) != 0 ) {
	return;
    }
    
    //
    // Set the zoom ratio and the packing.
    //

    {
	VLControlValue val;

	val.intVal = VL_TIMING_525_SQ_PIX;
	vlSetControl( myServer, myPath, mySource, VL_TIMING, &val );
	
	val.fractVal.numerator   = 1;
	val.fractVal.denominator = 4;
	vlSetControl( myServer, myPath, myDrain, VL_ZOOM, &val );
	
	val.intVal = VL_PACKING_RGBA_8;
	vlSetControl( myServer, myPath, myDrain, VL_PACKING, &val );
    }
    ...
    {
	VLControlValue val;
	vlGetControl( myServer, myPath, myDrain, VL_RATE, &val );
	printf( "Frame rate is: %d/%d\n",
		val.fractVal.numerator,
		val.fractVal.denominator );
	vlGetControl( myServer, myPath, myDrain, VL_CAP_TYPE, &val );
	// printf( "Capture type: %d\n", val.intVal );
	vlGetControl( myServer, myPath, myDrain, VL_SIZE, &val );
	printf( "Image size: %d x %d\n", val.xyVal.x, val.xyVal.y );
	assert( val.xyVal.x == width && val.xyVal.y == height );
	vlGetControl( myServer, myPath, myDrain, VL_PACKING, &val );
	// printf( "Packing: %d\n", val.intVal );
    }

    //
    // Create the buffer that will hold the frames
    //

    myBuffer = vlCreateBuffer( myServer, myPath, myDrain, BUFFER_FRAME_COUNT );
    if ( myBuffer == NULL ) {
	return;
    }
    {
	int status = vlRegisterBuffer( myServer, myPath, myDrain, myBuffer );
	if ( status != 0 ) {
	    return;
	}
    }
    ...
    {
	int status = vlBeginTransfer( myServer, myPath, 0, NULL );
	if ( status != 0 ) {
	    return;
	}
    }
    ...
    )
{
	...
	vlPutFree( myServer, myBuffer );
	myCurrentInfo = NULL;
    }
    

    if ( myBuffer != NULL ) {
	vlDestroyBuffer( myServer, myBuffer );
	myBuffer = NULL;
    }

    if ( myPath != -1 ) {
	int status = vlDestroyPath( myServer, myPath );
	assert( status == 0 );
	myPath = -1;
    }

    if ( myServer != NULL ) {
	int status = vlCloseVideo( myServer );
	assert( status == 0 );
	myServer = NULL;
    }
}
    ...
    )
{
    ...
    //
    // Get the MSC of the next frame that will come in.
    //

    myCurrentMSC = vlGetFrontierMSC( myServer, myPath, myDrain );
    
    //
    // Get the info pointer for the next frame.
    //

    myCurrentInfo = vlGetNextValid( myServer, myBuffer );
    while ( myCurrentInfo == NULL ) {
	sginap( 1 );
	myCurrentInfo = vlGetNextValid( myServer, myBuffer );
    }
    
    //
    // If this is the first time through, grab the sequence number of
    // the first frame, so that we can start counting at 0 for our
    // client. 
    //

    if ( myFirstFrame ) {
	myFirstFrame = FALSE;
	DMediaInfo* info = vlGetDMediaInfo( myServer, myBuffer, myCurrentInfo);
	myFirstMSC  = myCurrentMSC;
    }
}
...
stamp_t videoIn::getMSC(
    )
{
    assert( myCurrentInfo != NULL );
    
    return myCurrentMSC - myFirstMSC;
}
...
ABGRPixel* videoIn::getFrame(
    )
{
    assert( myCurrentInfo != NULL );
    
    return (ABGRPixel*) vlGetActiveRegion( myServer, myBuffer, myCurrentInfo );
}

////////////////////////////////////////////////////////////////////////
//
// testVideoIn implementation class
//
////////////////////////////////////////////////////////////////////////
...
public:
    ...
    testVideoIn( int width, int height );
    ~testVideoIn();
    ...
    VLBoolean getValid() { return TRUE; }
...
void testVideoIn::advance(
    )
{
    //
    // Advance the counter to the next frame
    //
    ...
    unsigned char* ptr = (unsigned char*) myBuffer;
...
ABGRPixel* testVideoIn::getFrame(
    )
{
    return (ABGRPixel*) myBuffer;
}

////////////////////////////////////////////////////////////////////////
//
// VideoIn interface class
//
////////////////////////////////////////////////////////////////////////

// #define FAKE_VIDEO_IN
...
VideoIn* VideoIn::create(
    int width,
    int height
    )
{
#ifdef FAKE_VIDEO_IN
    testVideoIn* result = new testVideoIn( width, height );
#else
    videoIn* result = new videoIn( width, height );
#endif
