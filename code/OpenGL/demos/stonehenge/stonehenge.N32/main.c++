...
Widget create_widgets(Widget parent)
{
  ...
  button = XmCreateToggleButton(menu_pane, "scale = 1:10", NULL, 0);
  XtManageChild(button);
  XtAddCallback(button, XmNvalueChangedCallback,
		(XtCallbackProc)timeSpeedCB, &speeds[2]);
  button = XmCreateToggleButton(menu_pane, "scale = 1:100", NULL, 0);
  XtManageChild(button);
  XtAddCallback(button, XmNvalueChangedCallback,
		(XtCallbackProc)timeSpeedCB, &speeds[3]);
  button = XmCreateToggleButton(menu_pane, "scale = 1:500", NULL, 0);
  XtManageChild(button);
  XtAddCallback(button, XmNvalueChangedCallback,
