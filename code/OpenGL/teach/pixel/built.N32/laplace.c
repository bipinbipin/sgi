...
main(int argc,char *argv[]) {
    ...
    if( argc<2 ) {
	fprintf(stderr,"usage: laplace [-n] inimage.rgb\n");
	exit(1);
    } 
    if( (image=iopen(argv[1],"r")) == NULL ) {
	fprintf(stderr,"laplace: can't open input file %s\n",argv[1]);
	exit(1);
    }
    xsize = image->xsize;
    ysize = image->ysize;
    zsize = image->zsize;
    if(zsize<3) {
	fprintf(stderr,"laplace: this is not an RGB image file\n");
	exit(1);
    }
	...
	case KeyPress: {
		...
		laplace();
		on ^= 1;
		if (on) glEnable(GL_CONVOLUTION_2D_EXT);
		else glDisable(GL_CONVOLUTION_2D_EXT);
		redraw = GL_TRUE;
		break;
	    }
	    break;
