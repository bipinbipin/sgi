...
makeArch()
{
	...
	for (i = 1;i < 100;i++)
	 {
	   rndnum_vert[i][0] = lrand48() / 1000000;
   ...
   myVertexProperty->vertex.setValues(0, 100, rndnum_vert);

   // Define the QuadMesh.
   SoQuadMesh *myQuadMesh = new SoQuadMesh;

   myQuadMesh->verticesPerRow = 20;
...
//   myDrawStyle-> linePattern.setValue(0xf0f0);
   ...
   //myViewer->setDrawStyle(SoXtViewer::STILL, SoXtViewer::VIEW_POINT);
   ...
   myViewer->setTitle("revamp");
   myViewer->setIconTitle("revamp");
   myViewer->show();
   myViewer->viewAll();
