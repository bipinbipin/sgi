...
#define USAGE \
"%s: [-f] [-t <window name>] [-d] [-I] [-v <videonode> ] [-b <yuv>] [-h]\n\
\t-f\tdisable forking\n\
\t-t\tset window title\n\
\t-d\tdebug mode\n\
...
main(int argc, char **argv)
{
     ...
     */
    while ((c = getopt(argc, argv, "t:dv:Ifb:h")) != EOF) 
    {
	switch(c) 
	{
	    case 'f':
    ...
    */
    ...
    if (bg_flag) {
	val.intVal = bg_y;
	vlSetControl(vlSvr, vlPath, blend_node, VL_MGV_BLEND_B_Y, &val);
	val.intVal = bg_u;
	vlSetControl(vlSvr, vlPath, blend_node, VL_MGV_BLEND_B_U, &val);
	val.intVal = bg_v;
	vlSetControl(vlSvr, vlPath, blend_node, VL_MGV_BLEND_B_V, &val);
	val.intVal = 1;
	vlSetControl(vlSvr, vlPath, blend_node, VL_MGV_BLEND_B_FLAT, &val);
    }

    val.intVal = 100;
    vlSetControl(vlSvr, vlPath, blend_node, VL_MGV_WIPE_SPEED, &val);
...
processXEvent(uint fd, void *source)
{
	...
	switch (ev.type) 
	{
		...
		{   /* Quit */
		    ...
		    old_val = val.intVal;
		    vlSetControl(vlSvr, vlPath, blend_node, VL_MGV_WIPE, &val);
		    break;
