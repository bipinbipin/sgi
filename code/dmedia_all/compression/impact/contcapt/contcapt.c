...
char *cmdargs = "b:c:C:deDfFE:g:hi:mn:oO:r:Rtv:wz:H:V:M:";
...
char *usage_m =
	...
	"       -e      no vl event in capture\n"
	"	-f	fast mode (no display)\n"
	"	-F	specifies field (or non-interleaved) mode\n"
	"       -C n[oninterleaved] specifies field (non-interleaved) mode\n"
	"       -C e[ven]    	    specifies even field mode\n"
	"       -C a[ll]            specifies all field mode\n"
	"       -C o[dd]            specifies odd field mode\n"
	"       -C i[nterleaved]    specifies frame (or interleaved) mode\n"
	"	-g wxh	frame geometry (ex. 640x480)\n"
	"	-i n	report statistics every n frames\n"
	"	-m	monochrome input\n"
	"	-M drn	memory drain (depends on hardware)\n"
	"	-n dev	device number\n"
        "       -o      Open GL Mode\n"
	"       -O hxv  frame offset (ex. 0x0)\n"
	"       -E f1xf2 extra vertical frame offset (ex. 0x0)\n"
	...
	"       -H d/d  set h aspect factor to fraction 'd/d'\n"
	"       -V d/d  set v aspect factor to fraction 'd/d'\n"
	"	-h	this help message\n"
;
...
int             hoffset = 0;
int             voffset = 0;
int             h_aspect_num = 1, h_aspect_denom = 1;
int             v_aspect_num = 1, v_aspect_denom = 1;
int             use_events=1;
int 		f1_extra=0;
int 		f2_extra=0;
...
getcmdargs(int argc, char **argv)
{
	...
	switch (c) 
	{
	    ...
	    case 'F':
                fieldmode = VL_CAPTURE_NONINTERLEAVED;
                break;

	    case 'C':
		len = strlen(optarg);
		if (len ==0) {
		    printf("Error: invalid Capture format.\n");
		    break;
		}
printf("len=%d arg=%s\n", len,optarg);
		if (strncasecmp(  "noninterleaved",optarg, len)==0){
                fieldmode = VL_CAPTURE_NONINTERLEAVED;
		} else if (strncasecmp(  "even", optarg,len)==0){
                fieldmode = VL_CAPTURE_EVEN_FIELDS;
		} else if (strncasecmp(  "all", optarg,len)==0){
                fieldmode = VL_CAPTURE_FIELDS;
		} else if (strncasecmp(  "odd", optarg,len)==0){
                fieldmode = VL_CAPTURE_ODD_FIELDS;
		} else if (strncasecmp(  "interleaved", optarg,len)==0){
                fieldmode = VL_CAPTURE_INTERLEAVED;
		} else {
		    printf("Error: invalid Capture format %s.\n", optarg);
		}
		break;
	    ...
	    case 'O':
	    	...
	    	if (!xloc)
	    	{
	    		printf("Error: invalid offset format, using default size\n");
	    		break;
	    	}
	    	xlen = len - strlen(xloc);
	    	if (xlen < 1 || xlen > 4)
	    	{
	    		printf("Error: invalid h offset, using default h offset\n");
	    		break;
	    	}
	    	numbuf = strtok(optarg,"x");
	    	hoffset = atoi(numbuf);
	    	...
	    	if (ylen < 1 || ylen > 4)
	    	{
	    		printf("Error: invalid v offset, using default v offset\n");
	    		break;
	    	}
	    	numbuf = strtok(NULL,"");
	    	voffset = atoi(numbuf);
	    ...
	    case 'E':
	    	...
	    	if (xlen < 1 || xlen > 4)
	    	{
	    		printf("Error: invalid f1 offset, using zero f1 offset\n");
	    		break;
	    	}
	    	numbuf = strtok(optarg,"x");
	    	f1_extra = atoi(numbuf);

	    	ylen = len - xlen -1;
	    	if (ylen < 1 || ylen > 4)
	    	{
	    		printf("Error: invalid f2 offset, using zero f2 offset\n");
	    		break;
	    	}
	    	numbuf = strtok(NULL,"");
	    	f2_extra = atoi(numbuf);
	    	break;
	    ...
	    break;
     	    ...
     	    case 'H':
		if (sscanf(optarg,"%d/%d", &h_aspect_num, &h_aspect_denom) != 2 ||
			    !h_aspect_num || !h_aspect_denom)
		{
			fprintf(stderr, "%s: ERROR: aspect format <num>/<denom>",
			    _progName);
			exit(1);
		}
		break;
	    ...
	    case 'V':
		if (sscanf(optarg,"%d/%d", &v_aspect_num, &v_aspect_denom) != 2 ||
		    !v_aspect_num || !v_aspect_denom)
		{
			fprintf(stderr, "%s: ERROR: aspect format <num>/<denom>",
			    _progName);
			exit(1);
...
main(int argc, char **argv)
{
    ...
    drn = vlGetNode(vlSvr, VL_DRN, VL_MEM, memdrn);
    ...
    /* Set the h aspect ratio */
    val.fractVal.numerator = h_aspect_num;
    val.fractVal.denominator = h_aspect_denom;

    vlSetControl(vlSvr, vlPath, drn,  VL_MGC_HASPECT, &val);
    vlGetControl(vlSvr, vlPath, drn,  VL_MGC_HASPECT, &val);
    Debug("h_aspect = %d/%d\n", val.fractVal.numerator,
        val.fractVal.denominator);

    if (f1_extra){
    	val.intVal= f1_extra;
    	vlSetControl(vlSvr, vlPath, drn,  VL_MGC_F1_EXTRA_OFFSET, &val);
    	val.intVal= -100;
    	vlGetControl(vlSvr, vlPath, drn,  VL_MGC_F1_EXTRA_OFFSET, &val);
    	if (val.intVal!= f1_extra) printf("did not set f1_extra %d\n", val.intVal);

    }

    if (f2_extra){
    	val.intVal= f2_extra;
    	vlSetControl(vlSvr, vlPath, drn,  VL_MGC_F2_EXTRA_OFFSET, &val);
    	val.intVal= -100;
    	vlGetControl(vlSvr, vlPath, drn,  VL_MGC_F2_EXTRA_OFFSET, &val);
    	if (val.intVal!= f2_extra) printf("did not set f2_extra %d\n", val.intVal);
    }

    /* Set the zoom ratio */
    val.fractVal.numerator = v_aspect_num;
    val.fractVal.denominator = v_aspect_denom;
    Debug("try v_aspect = %d/%d\n", val.fractVal.numerator,
        val.fractVal.denominator);

    vlSetControl(vlSvr, vlPath, drn, VL_MGC_VASPECT, &val);
    vlGetControl(vlSvr, vlPath, drn, VL_MGC_VASPECT, &val);
    Debug("v_aspect = %d/%d\n", val.fractVal.numerator,
        val.fractVal.denominator);
    vlGetControl(vlSvr, vlPath, drn, VL_RATE, &val);
    if ((val.fractVal.numerator == 25)||
       (val.fractVal.numerator == 50)){
    }
    ...
    vlSelectEvents(vlSvr, vlPath, VLTransferCompleteMask|
	    ...
	    vlSelectEvents(vlSvr, vlPath, VLStreamPreemptedMask|VLTransferFailedMask);
	...
	if (val.intVal != fieldmode){
	    fprintf(stderr,"Capture type not set to %d but is %d\n",
		fieldmode, val.intVal);
	    if (val.intVal==VL_CAPTURE_NONINTERLEAVED)
		 fprintf(stderr,"Field (non-interleaved) mode\n");
	    else if (val.intVal==VL_CAPTURE_INTERLEAVED)
		 fprintf(stderr,"Frame (interleaved) mode\n");
	    else if (val.intVal==VL_CAPTURE_EVEN_FIELDS)
		 fprintf(stderr,"Field (even) mode\n");
	    else if (val.intVal==VL_CAPTURE_ODD_FIELDS)
		 fprintf(stderr,"Field (odd) mode\n");
	}
	if (val.intVal==VL_CAPTURE_NONINTERLEAVED){ Debug("Field (non-interleaved) mode\n");}
	else if (val.intVal==VL_CAPTURE_INTERLEAVED){ Debug("Frame (interleaved) mode\n");}
	else if (val.intVal==VL_CAPTURE_EVEN_FIELDS){ Debug("Field (even) mode\n");}
	else if (val.intVal==VL_CAPTURE_ODD_FIELDS){ Debug("Field (odd) mode\n");}
	fieldmode= val.intVal;
	vlGetControl(vlSvr, vlPath, drn, VL_RATE, &val);
	if (fieldmode==VL_CAPTURE_INTERLEAVED) {
	...
	*/
    ...
    /* Main event loop */
    if (use_events) vlMainLoop();
    else{
	Debug("no events mode\n");
	myevent();
    }
}

void
processVLEvent2(uint fd, void *source, VLEvent ev)
{
    int count;
    static int frameNum;
...
myevent(){
	...
	int vlbfd=vlBufferGetFd(transferBuf);
        int vlfd=vlConnectionNumber(vlSvr);
	while (1){
		...
		myfds[2].fd=vlfd;
		myfds[2].events=POLLIN;
	
		poll(myfds,3,-1);
		if (myfds[0].revents) processXEvent(vlSvr,&myevent, NULL);
		if (myfds[2].revents) processVLEvent(vlfd, NULL);
		if (myfds[1].revents){
		        myevent.reason=VLTransferComplete;
			processVLEvent2(vlbfd, NULL, myevent);
			}
	}
}
