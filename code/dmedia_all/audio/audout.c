 ...
 * Audout
 *
 * A small program to read interleaved data on stdin
 * and write it to an audio device.
 *
 * Audout takes several arguments:
...

{
	...
	while ((x = read(0,buf,bufbytes)) > 0) {
	    if (x % (nchans * bytes_per_word)) {
		fprintf(stderr, "data left over at end of read\n");
	    }
	    alWriteFrames(p,buf,(x / (nchans * bytes_per_word)));
	}
