...
void
tipTheBalance( void *userData, SoEventCallback *eventCB)
{
   ...
   if (SO_KEY_PRESS_EVENT(ev, RIGHT_ARROW) ||
      ...
      SoShapeKit  *sun;
      SbRotation  startRot, rayIncrement, stringIncrement;
...
//     ray2 = (SoShapeKit *)  sun->getPart("childList[1]",TRUE);
      ...
      if (SO_KEY_PRESS_EVENT(ev, RIGHT_ARROW)) {
         rayIncrement.setValue(SbVec3f(0, 0, 1), -.1);
      ...
      xf->rotation.setValue(startRot *  rayIncrement);
...
main(int , char **argv)
{
   ...
   myScene->setPart("cameraList[1]", new SoCameraKit);
   myScene->set("cameraList[1] { orientation 0 1 0 0 }");
   myScene->setPart("cameraList[2]", new SoCameraKit);
