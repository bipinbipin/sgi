...
buildScene()
{
   ...
   zNotch->rotation.setValue(SbVec3f(0, 1, 0.5), 60);

   SoRotor *partRotor = new SoRotor;
   
  for (j = 0; j < (PARTS_PER_LOOP/2); j++) {
   	...
   	for (i = 1; i <= PARTS_PER_LOOP; i++) {
		...
		g->addChild(k);
		partRotor->rotation.setValue(SbVec3f((i/10), 1, 0), 0); // z axis
		partRotor->speed = (0.01 * i);
   ...
   groupRotor->rotation.setValue(SbVec3f(0.5, 1, 0), 90);
   groupRotor->speed = 0.007;
   g->addChild(groupRotor);
